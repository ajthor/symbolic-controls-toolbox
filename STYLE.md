...
- Add a `Methodology:` section to the help instead of explaining methodology in the comments.

- Use an leading empty line in help documentation.
  > For example,
  > - Use
  >   ```matlab
  >   function Y = work(X)
  >   %WORK Do work.
  >   %
  >   %   The function WORK does some work on X and returns Y.
  >   ```
  >   instead of
  >   ```matlab
  >   function Y = work(X)
...
- Parameter validation functions should use `validateattributes` whenever possible.
  > For example,
  > - Use
  >   ```matlab
  >   function Y = work(X)
