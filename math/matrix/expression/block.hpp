#ifndef MATH_MATRIX_EXPRESSION_BLOCK_HPP
#define MATH_MATRIX_EXPRESSION_BLOCK_HPP

#include <math/matrix/block/block.hpp>
...
inline void BlockMatrix<T>::apply_add(const BlockMatrix<DT> &rhs) {
  MATH_ASSERT(n_ == (~rhs).nrows());
  MATH_ASSERT(m_ == (~rhs).ncols());

  for(size_t i = 0; i < n_; i++) {
    for(size_t j = 0; j < m_; j++) {
      MATH_ASSERT(v_[i*m_ + j].nrows() == (~rhs)[i*m_ + j]->nrows());
      MATH_ASSERT(v_[i*m_ + j].ncols() == (~rhs)[i*m_ + j]->ncols());
...
#endif /* end of include guard: MATH_MATRIX_EXPRESSION_BLOCK_HPP */
